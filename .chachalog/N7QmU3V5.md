...
All deprecated classes related to image service are marked with `@Deprecated(since = "8.2.4.0", forRemoval = true)` and recommend using the OSGi service from `org.jahia.bundles.core.services.images`.
...
- All Core usage now first look for OSGi service before falling back to Spring implementation (required in case of a custom image service usage)
