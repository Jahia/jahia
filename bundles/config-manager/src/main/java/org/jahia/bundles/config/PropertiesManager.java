...
public class PropertiesManager {
    ...
    private class PropertiesValuesImpl implements PropertiesValues {
        ...
        private void initProps() {
            this.currentProps = path.length() == 0 ? props : props.keySet().stream().filter(k -> k.startsWith(path + delimiter)).collect(Collectors.toMap(s -> s.substring(path.length() + 1), props::get));
        }
