...
public class RestoreConfigurations implements Action {
    ...
    public Object execute() throws Exception {
        ...
        Map<String, ConfigService.ConfigType> filteredConfigs = configs.keySet().stream()
                .filter(k -> types.contains(configs.get(k)))
                .collect(Collectors.toMap(Function.identity(), configs::get));
        System.out.println("Restoring config : " + types);

        Collection<String> restored = configService.restoreConfigurationsFromJCR(types);
