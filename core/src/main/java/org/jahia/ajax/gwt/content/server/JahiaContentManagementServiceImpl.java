...
// --------------------- Interface JahiaContentManagementServiceAsync ---------------------
    ...
    public RpcMap saveNode(GWTJahiaNode node, GWTJahiaNodeACL acl,
        ...
        try {
            ...
            throw new GWTJahiaServiceException(Messages.getInternalWithArguments("label.gwt.error.could.not.save.node", getUILocale(), node.getPath(), e.getLocalizedMessage()));
        }
...
//        setLock(Arrays.asList(node.getPath()), false);
    ...
    public void saveModule(String moduleId, String message) throws GWTJahiaServiceException {
        ...
        if (noChanges) {
            ...
            logger.error("Cannot synchronize module into sources", exception);
            throw exception;
        }
    }
    ...
    public List<GWTJahiaNode> getPortalNodes(String targetAreaName) {
        List<GWTJahiaNode> nodes = new ArrayList<GWTJahiaNode>();
        try {
            Query q = retrieveCurrentSession().getWorkspace().getQueryManager().createQuery("select * from [jnt:contentFolder] as l where localname()='" + targetAreaName + "' and isdescendantnode(l,['/sites'])", Query.JCR_SQL2);
