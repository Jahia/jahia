...
public class WorkflowHelper {
    ...
    public GWTJahiaWorkflowInfo getWorkflowInfo(String path, JCRSessionWrapper session) throws GWTJahiaServiceException {
        try {
            ...
            Map<String, List<WorkflowDefinition>> wfs = service.getPossibleWorkflows(node);
            for (Map.Entry<String, List<WorkflowDefinition>> entry : wfs.entrySet()) {
                for (WorkflowDefinition workflow : entry.getValue()) {
                    GWTJahiaWorkflowDefinition w = new GWTJahiaWorkflowDefinition();
                    w.setProvider(entry.getKey());
                ...
                for (Workflow workflow : entry.getValue()) {
                    for (WorkflowAction workflowAction : workflow.getAvailableActions()) {
                        if (workflowAction instanceof WorkflowTask) {
                            ...
                            action.setId(((WorkflowTask) workflowAction).getId());
                            Set<String> outcomes = ((WorkflowTask)workflowAction).getOutcomes();
