        ...
        if (referencedNode.getUUID() != null) {
            ...
            if (referencedNode instanceof JCRPortletNode) {
                try {
                    ...
                    PortletWindow window = ServicesRegistry.getInstance().getApplicationsManagerService().getPortletWindow(entryPointInstance, uuid, paramBean.getUser(), paramBean.getRealRequest(), paramBean.getResponse(), paramBean.getContext());
                    PortletWindowBean portletWindowBean = new PortletWindowBean(paramBean.getUser(), paramBean.getRealRequest(), window);
            ...
            return new GWTJahiaLayoutItem(jcrLayoutItemNode.getUUID(), navigation.getGWTJahiaNode(referencedNode), viewModeUrl, editModeUrl, helpModeUrl, column, row, status, currentPortletMode);
        } else {
            return null;
        }
