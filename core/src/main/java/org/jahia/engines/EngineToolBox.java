...
public class EngineToolBox {
    ...
    private static final org.apache.log4j.Logger logger =
            org.apache.log4j.Logger.getLogger (EngineToolBox.class);
    ...
    public boolean processFieldTypes (final JahiaField theField,
        ...
        try {
            ...
            final FieldSubEngine engine = (FieldSubEngine) thisMethod.invoke(
                    null, args);

            out = engine.handleField(jParams, new Integer(mode), engineMap);

            if (logger.isDebugEnabled()) {
                logger.debug("Editing "
                        + fieldEngineName.substring(fieldEngineName
                                .lastIndexOf(".")) + " !");
            }

        } catch (ClassNotFoundException cnfe) {
            throw new JahiaException("EngineToolBox:processFieldTypes",
                    "Class not found!", JahiaException.PAGE_ERROR,
                    JahiaException.CRITICAL_SEVERITY, cnfe);

        } catch (NoSuchMethodException nsme) {
            throw new JahiaException("EngineToolBox:processFieldTypes",
                    "Method not found!", JahiaException.PAGE_ERROR,
                    JahiaException.CRITICAL_SEVERITY, nsme);

        } catch (IllegalAccessException iae) {
            throw new JahiaException("EngineToolBox:processFieldTypes",
                    "Illegal access", JahiaException.PAGE_ERROR,
                    JahiaException.CRITICAL_SEVERITY, iae);

        } catch (InvocationTargetException ite) {

            throw new JahiaException("EngineToolBox:processFieldTypes",
                    "InvocationTarget exception", JahiaException.PAGE_ERROR,
                    JahiaException.CRITICAL_SEVERITY, ite);
        }
     ...
     */
    public void displayScreen (final ProcessingContext jParams, final Map<String, Object> engineMap)
            throws JahiaException {
    ...
    public void loadLogData (final ProcessingContext jParams, 
            final int objectType, final Map<String, Object> engineMap)
            throws JahiaException {
        ...
        final String objectName = JahiaObjectTool.getInstance ().getObjectName (objectType,
                engineMap);
        ...
        if (registry != null) {
            ...
            if (mAuditLogManager == null) {
                throw new JahiaException (MSG_INTERNAL_ERROR, "Properties Engine could not get the Audit Log Manager Service instance.",
                        JahiaException.SERVICE_ERROR, JahiaException.CRITICAL_SEVERITY);
            }
        } else {
            throw new JahiaException (MSG_INTERNAL_ERROR, "Properties Engine could not get the Service Registry instance.",
                    JahiaException.REGISTRY_ERROR, JahiaException.CRITICAL_SEVERITY);
        }

        // logs
        engineMap.put ("flushLogs", new Integer (flushLogs));
        engineMap.put ("sendLogsURL",
                ManageLogs_Engine.getInstance ().renderLink (jParams, ""));
        engineMap.put ("objectTypeName",
                JahiaObjectTool.getInstance ().getObjectTypeName (objectType));
        engineMap.put ("objectIDObj", new Integer (objectID));
        ...
        switch (flushLogs) {
            case 1:
                output = registry.getJahiaFetcherService ().fetchServlet ((ParamBean)jParams,
                        JSP_FLUSHCONFIRM);
                break;
            ...
            default:
                final List logData = mAuditLogManager.getLog (objectType, objectID,
                        jParams);
