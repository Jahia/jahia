 ...
 *
 * @hibernate.subclass discriminator-value="org.jahia.hibernate.model.indexingjob.JahiaPageIndexingJob"
 */
public class JahiaPageIndexingJob extends JahiaIndexingJob {

    private Integer pageId;
    private Integer siteId = new Integer(0);

    public JahiaPageIndexingJob(){
        super();
        setClassName(getClass().getName());
    }

    public JahiaPageIndexingJob(int pageId, long date){
        this();
        setPageId(new Integer(pageId));
        setDate(new Long(date));
    }

    /**
     * @hibernate.property column="pageid_indexingjob"
     * @return
     */
    ...
    public Integer getSiteId() {
        if ( siteId.intValue() == 0 ){
            if ( pageId != null){
                try {
                    ContentPage page = ContentPage.getPage(pageId.intValue());
                    siteId = new Integer(page.getSiteID());
                } catch ( Exception t ){
                }
            }
        }
    ...
    public void execute(JahiaUser user){
        if ( pageId != null ){
            ServicesRegistry.getInstance().getJahiaSearchService()
                .indexPage(pageId.intValue(),user,false,false,null);
        }
    }
    ...
    public void prepareBatchIndexation(List toRemove, List toAdd, JahiaUser user){
        List docs = ServicesRegistry.getInstance().getJahiaSearchService()
                .getIndexableDocumentsForPage(this.pageId.intValue(), user);
    ...
    public boolean equals(Object obj) {
        ...
        if (obj != null && this.getClass() == obj.getClass()) {
            final JahiaPageIndexingJob castOther = (JahiaPageIndexingJob) obj;
    ...
    public Object clone() throws CloneNotSupportedException {
        JahiaPageIndexingJob job = new JahiaPageIndexingJob();
