...
/**
 * Wrappers around <code>javax.jcr.Node</code> to be able to inject Jahia
 * specific actions.
 * 
 * @author toto
 */
public class JCRNodeWrapperImpl extends JCRItemWrapperImpl implements
		JCRNodeWrapper {
	protected static final Logger logger = org.slf4j.LoggerFactory
			.getLogger(JCRNodeWrapper.class);
	...
	protected JCRNodeWrapperImpl(Node objectNode, String path,
			JCRNodeWrapper parent, JCRSessionWrapper session,
	 ...
	 * {@inheritDoc}
	 */
	public JCRNodeWrapper getParent() throws ItemNotFoundException,
		...
		try {
			if (localPath.equals("/")
					|| localPath.equals(provider.getRelativeRoot())) {
				if (provider.getMountPoint().equals("/")) {
					throw new ItemNotFoundException();
				}
				return (JCRNodeWrapper) session.getItem(StringUtils
						.substringBeforeLast(provider.getMountPoint(), "/"));
			} else {
				return (JCRNodeWrapper) session.getItem(StringUtils
						.substringBeforeLast(getPath(), "/"));
			}
					...
					if (n.hasNode("j:acl")) {
						...
						while (aces.hasNext()) {
							...
							if (ace.isNodeType("jnt:ace")) {
								String principal = ace.getProperty(
										"j:principal").getString();
								String type = ace.getProperty("j:aceType")
										.getString();
								if (!ace.hasProperty(Constants.J_ROLES)) {
									continue;
								}
								Value[] roles = ace.getProperty(
										Constants.J_ROLES).getValues();

								if (!results.containsKey(principal)) {
									results.put(principal,
											new ArrayList<String[]>());
								}

								for (Value role : roles) {
									results.get(principal).add(
											new String[] { n.getPath(), type,
													role.getString() });
								}
							}
						}
						if (acl.hasProperty("j:inherit")
								&& !acl.getProperty("j:inherit").getBoolean()) {
							return results;
						}
					}
					n = n.getParent();
	 ...
	 * {@inheritDoc}
	 */
	public Map<String, List<JCRNodeWrapper>> getAvailableRoles()
		...
		while (ni.hasNext()) {
			...
			if (role.isNodeType("jnt:role")) {
				...
				if (role.hasProperty("j:hidden")
						&& role.getProperty("j:hidden").getBoolean()) {
					// skip
				} else if (role.hasProperty("j:nodeTypes")) {
					Value[] values = role.getProperty("j:nodeTypes")
							.getValues();
	...
	public boolean hasPermission(String perm) {
		try {
			...
			if (accessControlManager != null) {
				// List<Privilege> privileges = convertPermToPrivileges(perm,
				// accessControlManager);

				return accessControlManager.hasPrivileges(localPath,
						new Privilege[] { accessControlManager
								.privilegeFromName(perm) });
			}
			return true;
	...
	public AccessControlManager getAccessControlManager()
		...
		// this is not a Jackrabbit implementation, we will use the new JCR 2.0
		// API instead.
		...
		try {
			...
			if (accessControlManager != null) {
				...
				for (Privilege privilege : p) {
					...
					for (Privilege privilege1 : privilege
	...
	public BitSet getPermissionsAsBitSet() {
		...
		try {
			AccessControlManager accessControlManager = getAccessControlManager();
			List<Privilege> app = Arrays.asList(accessControlManager
					.getPrivileges(localPath));
			List<Privilege> pr = Arrays.asList(accessControlManager
					.getSupportedPrivileges(localPath));
	 ...
	 * {@inheritDoc}
	 */
	public boolean grantRoles(String principalKey, Set<String> roles)
			throws RepositoryException {
	 ...
	 * {@inheritDoc}
	 */
	public boolean denyRoles(String principalKey, Set<String> roles)
			throws RepositoryException {
		Map m = new HashMap<String, String>();
		for (String role : roles) {
			m.put(role, "DENY");
	 ...
	 * {@inheritDoc}
	 */
	public boolean changeRoles(String principalKey, Map<String, String> roles)
			throws RepositoryException {
		if (!objectNode.isCheckedOut()
				&& objectNode.isNodeType(Constants.MIX_VERSIONABLE)) {
			objectNode.getSession().getWorkspace().getVersionManager()
					.checkout(objectNode.getPath());
		}
		...
		if (aceg == null) {
			aceg = acl.addNode("GRANT_" + principalKey.replace(':', '_'),
					"jnt:ace");
		...
		if (aced == null) {
			aced = acl.addNode("DENY_" + principalKey.replace(':', '_'),
					"jnt:ace");
		...
		if (aceg.hasProperty(Constants.J_ROLES)) {
			final Value[] values = aceg.getProperty(Constants.J_ROLES)
					.getValues();
			for (Value value : values) {
		...
		if (aced.hasProperty(Constants.J_ROLES)) {
			final Value[] values = aced.getProperty(Constants.J_ROLES)
					.getValues();
			for (Value value : values) {
				final String s = value.getString();
				if (!gr.contains(s) && !den.contains(s)) {
	 ...
	 * {@inheritDoc}
	 */
	public boolean revokeRolesForPrincipal(String principalKey)
			throws RepositoryException {
		Node acl = getOrCreateAcl();
	...
	public boolean getAclInheritanceBreak() throws RepositoryException {
		if (objectNode.hasNode("j:acl")) {
			...
			return acl.hasProperty("j:inherit")
					&& !acl.getProperty("j:inherit").getBoolean();
		}
		return false;
	}
	 ...
	 * {@inheritDoc}
	 */
	public JCRNodeWrapper createCollection(String name)
			throws RepositoryException {
	 ...
	 * {@inheritDoc}
	 */
	public JCRNodeWrapper uploadFile(String name, final InputStream is,
		...
		if (file != null) {
			...
			logger.error("can't write to file " + name
	...
	public JCRNodeWrapper addNode(String name) throws RepositoryException {
		...
		return provider
				.getNodeWrapper(n, buildSubnodePath(name), this, session);
	}
	...
	public JCRNodeWrapper addNode(String name, String type, String identifier,
			Calendar created, String createdBy, Calendar lastModified,
		...
		if (objectNode instanceof NodeImpl) {
			...
			jrSession.getNodeTypeInstanceHandler().setLastModifiedBy(
					lastModifiedBy);
			try {
				if (identifier != null) {

					org.jahia.services.content.nodetypes.Name jahiaName = new org.jahia.services.content.nodetypes.Name(
							name, NodeTypeRegistry.getInstance()
									.getNamespaces());
					Name qname = NameFactoryImpl.getInstance().create(
							jahiaName.getUri() == null ? ""
									: jahiaName.getUri(),
							jahiaName.getLocalName());
					Name typeName = null;
					if (type != null) {
						org.jahia.services.content.nodetypes.Name jahiaTypeName = NodeTypeRegistry
								.getInstance().getNodeType(type)
								.getNameObject();
						typeName = NameFactoryImpl.getInstance().create(
								jahiaTypeName.getUri(),
								jahiaTypeName.getLocalName());
					}
					Node child = ((NodeImpl) objectNode).addNode(qname,
							typeName, org.apache.jackrabbit.core.id.NodeId
									.valueOf(identifier));
					return provider.getNodeWrapper(child,
							buildSubnodePath(name), this, session);
				} else {
	...
	public String getUrl() {
		try {
			if (isNodeType(Constants.JAHIANT_FILE)) {
				return provider.getHttpPath() + "/"
						+ getSession().getWorkspace().getName()
						+ Text.escapePath(getPath());
			} else {
				String path = JCRSessionFactory.getInstance()
						.getCurrentServletPath();
				if (path == null) {
					path = "/cms/render";
				}
				return Jahia.getContextPath() + path + "/"
						+ getSession().getWorkspace().getName() + "/"
						+ getSession().getLocale() + Text.escapePath(getPath())
						+ ".html";
			}
		} catch (RepositoryException e) {
	...
	public String getWebdavUrl() {
		try {
			return Jahia.getContextPath() + provider.getWebdavPath() + "/"
					+ session.getWorkspace().getName() + objectNode.getPath();
		} catch (RepositoryException e) {
	...
	public NodeIterator getNodes() throws RepositoryException {
		...
		if (provider.getService() != null) {
			Map<String, JCRStoreProvider> mountPoints = provider
					.getSessionFactory().getMountPoints();
			for (Map.Entry<String, JCRStoreProvider> entry : mountPoints
					.entrySet()) {
				if (!entry.getKey().equals("/")) {
					String mpp = entry.getKey().substring(0,
					...
					if (mpp.equals(getPath())) {
						...
						final Node node = session.getProviderSession(
								storeProvider).getNode(
								root.length() == 0 ? "/" : root);
						list.add(storeProvider.getNodeWrapper(node, "/", this,
								session));
					}
				}
			}
		...
		while (ni.hasNext()) {
			...
			if (session.getLocale() == null
					|| !node.getName().startsWith("j:translation_")) {
				try {
					JCRNodeWrapper child = provider.getNodeWrapper(node,
							buildSubnodePath(node.getName()), this, session);
					list.add(child);
				} catch (ItemNotFoundException e) {
	...
	public NodeIterator getNodes(String name) throws RepositoryException {
		...
		if (provider.getService() != null) {
			...
			if (mountPoints.containsKey(getPath() + "/" + name)) {
				JCRStoreProvider storeProvider = mountPoints.get(getPath()
						+ "/" + name);
				final Node node = session.getProviderSession(storeProvider)
						.getNode(storeProvider.getRelativeRoot());
	 ...
	 * {@inheritDoc}
	 */
	public JCRNodeWrapper getNode(String s) throws PathNotFoundException,
			RepositoryException {
		if (objectNode.hasNode(s)) {
			if (!s.contains("/")) {
				return provider.getNodeWrapper(objectNode.getNode(s),
						buildSubnodePath(s), this, session);
			} else {
	...
	public String getName() {
		try {
			if ((objectNode.getPath().equals("/") || objectNode.getPath()
					.equals(provider.getRelativeRoot()))
					&& provider.getMountPoint().length() > 1) {
	...
	public ExtendedNodeType[] getMixinNodeTypes() throws RepositoryException {
		...
		for (NodeType nodeType : objectNode.getMixinNodeTypes()) {
			l.add(NodeTypeRegistry.getInstance()
					.getNodeType(nodeType.getName()));
		}
	 ...
	 * {@inheritDoc}
	 */
	public void addMixin(String s) throws NoSuchNodeTypeException,
	 ...
	 * {@inheritDoc}
	 */
	public void removeMixin(String s) throws NoSuchNodeTypeException,
			VersionException, ConstraintViolationException, LockException,
			RepositoryException {
		checkLock();
		applicablePropertyDefinition.clear();
	 ...
	 * {@inheritDoc}
	 */
	public boolean canAddMixin(String s) throws NoSuchNodeTypeException,
			RepositoryException {
		try {
			checkLock();
	...
	public ExtendedNodeDefinition getDefinition() throws RepositoryException {
		...
		if (definition.getName().equals("*")) {
			for (ExtendedNodeDefinition d : nt
					.getUnstructuredChildNodeDefinitions().values()) {
				ExtendedNodeType[] requiredPrimaryTypes = d
	...
	public boolean isCollection() {
		return true;
		// try {
		// return objectNode.isNodeType("jmix:collection") ||
		// objectNode.isNodeType("nt:folder") ||
	...
	public Date getLastModifiedAsDate() {
		try {
			return objectNode.getProperty(Constants.JCR_LASTMODIFIED).getDate()
					.getTime();
		} catch (Exception e) {
		}
		return null;
	...
	public Date getContentLastModifiedAsDate() {
		try {
			...
			return content.getProperty(Constants.JCR_LASTMODIFIED).getDate()
					.getTime();
	...
	public Date getLastPublishedAsDate() {
		try {
			return objectNode.getProperty(Constants.LASTPUBLISHED).getDate()
					.getTime();
		} catch (Exception e) {
		}
		return null;
	...
	public Date getContentLastPublishedAsDate() {
		try {
			Node content = objectNode.getNode(Constants.JCR_CONTENT);
			return content.getProperty(Constants.LASTPUBLISHED).getDate()
					.getTime();
		} catch (PathNotFoundException pnfe) {
		} catch (RepositoryException e) {
		}
	...
	public Date getCreationDateAsDate() {
		try {
			return objectNode.getProperty(Constants.JCR_CREATED).getDate()
					.getTime();
		} catch (RepositoryException e) {
		}
		return null;
	...
	public String getModificationUser() {
		try {
			return objectNode.getProperty(Constants.JCR_LASTMODIFIEDBY)
					.getString();
		} catch (RepositoryException e) {
		}
		return null;
	...
	public String getPublicationUser() {
		try {
			return objectNode.getProperty(Constants.LASTPUBLISHEDBY)
					.getString();
		} catch (RepositoryException e) {
		}
		return null;
	...
	public String getLanguage() {
		...
		if (language == null && getSession().getLocale() != null) {
			try {
				language = getI18N(getSession().getLocale()).getProperty(
						"jcr:language").getString();
			} catch (Exception e) {
	...
	public List<Locale> getExistingLocales() throws RepositoryException {
		...
		while (ni.hasNext()) {
			Node n = ni.nextNode();
			r.add(org.jahia.utils.LanguageCodeConverters.languageCodeToLocale(n
					.getProperty("jcr:language").getString()));
		}
		return r;
	}
	...
	private boolean hasI18N(Locale locale, boolean fallback)
			throws RepositoryException {
		boolean b = (i18NobjectNodes != null && i18NobjectNodes
				.containsKey(locale))
				|| objectNode.hasNode("j:translation_" + locale);
		if (!b && fallback) {
			final Locale fallbackLocale = getSession().getFallbackLocale();
			if (fallbackLocale != null && fallbackLocale != locale) {
				b = (i18NobjectNodes != null && i18NobjectNodes
						.containsKey(fallbackLocale))
						|| objectNode
								.hasNode("j:translation_" + fallbackLocale);
			}
		}
		return b;
	}

	protected Node getI18N(Locale locale, boolean fallback)
			throws RepositoryException {
	...
	public Node getOrCreateI18N(Locale locale) throws RepositoryException {
		try {
			...
			Node t = objectNode.addNode("j:translation_" + locale,
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper getProperty(String name)
	...
	private JCRPropertyWrapper internalGetProperty(String name,
		...
		if (locale != null) {
			if (epd != null && epd.isInternationalized()) {
				try {
					...
					return new JCRPropertyWrapperImpl(this,
							localizedNode.getProperty(name), session, provider,
							epd, name);
				} catch (ItemNotFoundException e) {
					return new JCRPropertyWrapperImpl(this,
							objectNode.getProperty(name), session, provider,
							epd);
				}
			}
		}
		return new JCRPropertyWrapperImpl(this, objectNode.getProperty(name),
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, String value)
	...
	private String ensurePrefixedName(String name) {
		...
		return StringUtils.isEmpty(nameObj.getPrefix()) ? nameObj
				.getLocalName() : nameObj.getPrefix() + ":"
				+ nameObj.getLocalName();
	}

	/**
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Value value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		checkLock();
		...
		if (epd == null) {
			throw new ConstraintViolationException(
					"Couldn't find definition for property " + name);
		}

		if (value != null && PropertyType.UNDEFINED != epd.getRequiredType()
				&& value.getType() != epd.getRequiredType()) {
			// if the type doesn't match the required type, we attempt a
			// conversion.
			value = getSession().getValueFactory().createValue(
					value.getString(), epd.getRequiredType());
		}
		value = JCRStoreService.getInstance().getInterceptorChain()
				.beforeSetValue(this, name, epd, value);

		if (value instanceof ExternalReferenceValue) {
			String nodeIdentifier = value.getString();
			JCRNodeWrapper node = getSession().getNodeByIdentifier(
					nodeIdentifier);
		...
		if (locale != null) {
			if (epd != null && epd.isInternationalized()) {
				return new JCRPropertyWrapperImpl(this, getOrCreateI18N(locale)
						.setProperty(name, value), session, provider, epd, name);
			}
		}

		if (value == null) {
			objectNode.setProperty(name, value);
			return new JCRPropertyWrapperImpl(this, null, session, provider,
					epd);
		}
		return new JCRPropertyWrapperImpl(this, objectNode.setProperty(name,
				value), session, provider, epd);
	}

	/**
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Value value, int type)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		checkLock();
		hasPropertyCache.remove(name);
		...
		if (locale != null) {
			if (epd != null && epd.isInternationalized()) {
				return new JCRPropertyWrapperImpl(this, getOrCreateI18N(locale)
						.setProperty(name, value, type), session, provider,
						epd, name);
			}
		}
		...
		return new JCRPropertyWrapperImpl(this, objectNode.setProperty(name,
				value, type), session, provider, epd);
	}

	/**
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Value[] values)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		checkLock();
		hasPropertyCache.remove(name);
		...
		if (values != null) {
			for (int i = 0; i < values.length; i++) {
				if (values[i] != null
					...
					values[i] = getSession().getValueFactory().createValue(
							values[i].getString(), epd.getRequiredType());
				}
			}
		}

		values = JCRStoreService.getInstance().getInterceptorChain()
				.beforeSetValues(this, name, epd, values);

		if (locale != null) {
			if (epd != null && epd.isInternationalized()) {
				return new JCRPropertyWrapperImpl(this, getOrCreateI18N(locale)
						.setProperty(name, values), session, provider, epd,
						name);
			}
		}

		return new JCRPropertyWrapperImpl(this, objectNode.setProperty(name,
				values), session, provider, epd);
	}

	/**
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Value[] values, int type)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		final Locale locale = getSession().getLocale();
		hasPropertyCache.remove(name);
		...
		if (locale != null) {
			if (epd != null && epd.isInternationalized()) {
				return new JCRPropertyWrapperImpl(this, getOrCreateI18N(locale)
						.setProperty(name, values, type), session, provider,
						epd, name);
			}
		}

		return new JCRPropertyWrapperImpl(this, objectNode.setProperty(name,
				values, type), session, provider, epd);
	}

	/**
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, String[] values)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value[] v = null;
		if (values != null) {
			...
			for (int i = 0; i < values.length; i++) {
				if (values[i] != null) {
					v[i] = getSession().getValueFactory()
							.createValue(values[i]);
				} else {
					v[i] = null;
				}
			}
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, String[] values, int type)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value[] v = null;
		if (values != null) {
			...
			for (int i = 0; i < values.length; i++) {
				if (values[i] != null) {
					v[i] = getSession().getValueFactory().createValue(
							values[i], type);
				} else {
					v[i] = null;
				}
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, String value, int type)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = null;
		if (value != null) {
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, InputStream value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = null;
		if (value != null) {
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, boolean value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, double value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = getSession().getValueFactory().createValue(value);
		return setProperty(name, v);
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, long value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = getSession().getValueFactory().createValue(value);
		return setProperty(name, v);
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Calendar value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = null;
		if (value != null) {
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Node value)
			throws ValueFormatException, VersionException, LockException,
			RepositoryException {
		Value v = null;
		if (value != null) {
			...
			if (epd != null) {
				if (value.getClass().getName()
						.equals(getRealNode().getClass().getName())) {
					v = getSession()
							.getValueFactory()
							.createValue(
									value,
									epd.getRequiredType() == PropertyType.WEAKREFERENCE);
				} else {
	...
	private JCRPropertyWrapper setExternalReferenceProperty(String name,
			Node node, ExtendedPropertyDefinition epd)
			throws RepositoryException {
		internalSetExternalReferenceProperty(name, node.getIdentifier());

		Property nodeProperty = new ExternalReferencePropertyImpl(name, this,
				session, node.getIdentifier(), node);
		return new JCRPropertyWrapperImpl(this, nodeProperty, session,
				provider, epd);
	}

	private void internalSetExternalReferenceProperty(String name,
			String nodeIdentifier) throws RepositoryException {
		// we are creating a reference to a node that is in another repository,
		// we will use a special mixin for that.
		...
		if (!isNodeType(Constants.JAHIAMIX_EXTERNALREFERENCE)) {
			...
			if (nodeIdentifierProperty != null) {
				// We create an ArrayList because Arrays.asList returns a
				// non-mutable list
				nodeIdentifiers = new ArrayList<Value>(
						Arrays.asList(nodeIdentifierProperty.getValues()));
			} else {
				nodeIdentifiers = new ArrayList<Value>();
			}
			Property referenceProperty = getProperty(REFERENCE_PROPERTY_NAMES_PROPERTYNAME);
			if (referenceProperty != null) {
				...
				// non-mutable list
				referenceProperties = new ArrayList<Value>(
						Arrays.asList(referenceProperty.getValues()));
			} else {
				referenceProperties = new ArrayList<Value>();
			}

		}
		Value newNodeIdentifierValue = getSession().getValueFactory()
				.createValue(nodeIdentifier);
		if (!nodeIdentifiers.contains(newNodeIdentifierValue)) {
			nodeIdentifiers.add(newNodeIdentifierValue);
		}
		setProperty(REFERENCE_NODE_IDENTIFIERS_PROPERTYNAME,
				nodeIdentifiers.toArray(new Value[nodeIdentifiers.size()]));

		Value newPropertyReferenceValue = getSession().getValueFactory()
				.createValue(
						nodeIdentifier
		...
		setProperty(REFERENCE_PROPERTY_NAMES_PROPERTYNAME,
				referenceProperties.toArray(new Value[referenceProperties
						.size()]));
	}

	protected void removeExternalReferenceProperty(String name)
			throws ItemNotFoundException, RepositoryException {
		if (!isNodeType(Constants.JAHIAMIX_EXTERNALREFERENCE)) {
			// quick sanity check, but usually we will do this in this method's
			// caller
			return;
		}
		...
		Property nodeIdentifierProperty = getProperty(REFERENCE_NODE_IDENTIFIERS_PROPERTYNAME);
		List<Value> identifiers = new ArrayList<Value>(
				Arrays.asList(nodeIdentifierProperty.getValues()));
		if (identifiers.size() == 0) {
			nodeIdentifierProperty.remove();
			return;
		}
		Value externalReferenceIdValue = getSession().getValueFactory()
				.createValue(externalReferenceId);
		if (identifiers.contains(externalReferenceIdValue)) {
			identifiers.remove(externalReferenceIdValue);
		}
		if (identifiers.size() > 0) {
			setProperty(REFERENCE_NODE_IDENTIFIERS_PROPERTYNAME,
					identifiers.toArray(new Value[identifiers.size()]));
		} else {
			nodeIdentifierProperty.remove();
		...
		List<Value> propertyNames = new ArrayList<Value>(
				Arrays.asList(referencePropertyNames.getValues()));
		if (propertyNames.size() == 0) {
			referencePropertyNames.remove();
		}
		Value externalReferencePropertyValue = getSession().getValueFactory()
				.createValue(
						externalReferenceId
								+ EXTERNAL_IDENTIFIER_PROP_NAME_SEPARATOR
								+ name);
		...
		if (propertyNames.size() > 0) {
			setProperty(REFERENCE_NODE_IDENTIFIERS_PROPERTYNAME,
					propertyNames.toArray(new Value[propertyNames.size()]));
		} else {
			referencePropertyNames.remove();
			mustRemoveMixin = true;
		}
		...
		if (mustRemoveMixin) {
			// as the node can have multiple reference properties, we only
			// remove the mixin if all properties have
			// been removed.
	...
	private String findExternalReferenceIdFromPropertyName(String name)
			throws ItemNotFoundException, RepositoryException {
		Property referenceProperty = getProperty(REFERENCE_PROPERTY_NAMES_PROPERTYNAME);
		if (referenceProperty == null) {
			throw new ItemNotFoundException("Couldn't find "
					+ REFERENCE_PROPERTY_NAMES_PROPERTYNAME
					+ " property on node " + getPath());
		}
		Value[] referencePropNames = referenceProperty.getValues();
		for (Value referencePropName : referencePropNames) {
			String[] referencePropNameParts = referencePropName.getString()
					.split(EXTERNAL_IDENTIFIER_PROP_NAME_SEPARATOR);
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, Binary value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = null;
		if (value != null) {
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String name, BigDecimal value)
			throws ValueFormatException, VersionException, LockException,
			ConstraintViolationException, RepositoryException {
		Value v = null;
		if (value != null) {
	...
	private boolean internalHasProperty(String propertyName)
			throws RepositoryException {
		final Locale locale = getSession().getLocale();
	 ...
	 * {@inheritDoc}
	 */
	public JCRPropertyWrapper setProperty(String namespace, String name,
	...
	public boolean rename(String newName) throws RepositoryException {
		...
		// the following code is use to conserve the ordering when renaming a
		// node, we do this only if the parent
		// node is orderable.
		...
		this.objectNode = getSession().getProviderSession(getProvider())
				.getNode(localPath);
		if ((nodePositionFound)
	...
	public boolean copy(String dest, String name) throws RepositoryException {
		...
		boolean sameProvider = (provider.getKey().equals(node.getProvider()
				.getKey()));
		if (!sameProvider) {
			copy(node, name, true);
			node.save();
	 ...
	 * {@inheritDoc}
	 */
	public boolean copy(JCRNodeWrapper dest, String name,
			boolean allowsExternalSharedNodes) throws RepositoryException {
	...
	public boolean copy(JCRNodeWrapper dest, String name,
		...
		if (copy != null) {
			...
			if (hasProperty("jcr:language")) {
				copy.setProperty("jcr:language", getProperty("jcr:language")
						.getString());
			}
		...
		while (ni.hasNext()) {
			...
			if (source.isNodeType("mix:shareable")) {
				if (uuidMapping.containsKey(source.getIdentifier())) {
					...
					session.save();
					copy.clone(session.getNodeByUUID(uuidMapping.get(source
							.getIdentifier())), source.getName());
				} else if (allowsExternalSharedNodes) {
					copy.clone(source, source.getName());
				} else {
					source.copy(copy, source.getName(),
							allowsExternalSharedNodes, references);
				}
			} else {
	...
	public void copyProperties(JCRNodeWrapper destinationNode,
			Map<String, List<String>> references) throws RepositoryException {
		...
		while (props.hasNext()) {
			...
			try {
				if (!Constants.forbiddenPropertiesToCopy.contains(property
						.getName())) {
					if (property.getType() == PropertyType.REFERENCE
						...
						if (property.getDefinition().isMultiple()
							...
							for (Value value : values) {
								keepReference(destinationNode, references,
										property, value.getString());
							}
						} else {
							keepReference(destinationNode, references,
									property, property.getValue().getString());
						}
					}
					if (property.getDefinition().isMultiple()
							&& (property.isMultiple())) {
						destinationNode.setProperty(property.getName(),
								property.getValues());
					} else {
						destinationNode.setProperty(property.getName(),
								property.getValue());
				...
				logger.warn("Unable to copy property '" + property.getName()
						+ "'. Skipping.", e);
			}
		}
	}

	private void keepReference(JCRNodeWrapper destinationNode,
			Map<String, List<String>> references, Property property,
			String value) throws RepositoryException {
		if (!references.containsKey(value)) {
			references.put(value, new ArrayList<String>());
		}
		references.get(value).add(
				destinationNode.getIdentifier() + "/" + property.getName());
	}
	...
	public void remove() throws VersionException, LockException,
		...
		if (!getSession().getWorkspace().getName()
				.equals(Constants.LIVE_WORKSPACE)
				&& provider.getMountPoint().equals("/")) {
			try {
				JCRNodeWrapper parent = getParent();
				...
				if (!parent.isNodeType("jmix:deletedChildren")) {
					...
					parent.getProperty("j:deletedChildren").addValue(
							getIdentifier());
				}
			} catch (ItemNotFoundException e) {
				// no live
	...
	public boolean lockAndStoreToken(String type) throws RepositoryException {
		String l = getSession().isSystem() ? " system " : getSession()
				.getUserID();
	 ...
	 * @param type
	 */
	public boolean lockAndStoreToken(String type, String userID)
			throws RepositoryException {
		...
		if (session.getLocale() != null
				&& !isNodeType(Constants.JAHIANT_TRANSLATION)) {
			Node trans = null;
			try {
	...
	private void addLockTypeValue(final Node objectNode, String l)
			throws RepositoryException {
		getSession().checkout(objectNode);

		if (objectNode.hasProperty(Constants.JAHIA_LOCKTYPES)) {
			Property property = objectNode
					.getProperty(Constants.JAHIA_LOCKTYPES);
			...
			newValues[oldValues.length] = getSession().getValueFactory()
					.createValue(l);
			property.setValue(newValues);
		} else {
			objectNode.setProperty(Constants.JAHIA_LOCKTYPES,
					new String[] { l });
		}
	}
	...
	public List<Locale> getLockedLocales() throws RepositoryException {
		...
		while (ni.hasNext()) {
			Node n = ni.nextNode();
			if (n.isLocked()) {
				r.add(org.jahia.utils.LanguageCodeConverters
						.languageCodeToLocale(n.getProperty("jcr:language")
								.getString()));
			}
		}
		return r;
	...
	public List<Locale> getLockedLocalesForUserAndType(String type)
			throws RepositoryException {
		List<Locale> r = new ArrayList<Locale>();
		NodeIterator ni = objectNode.getNodes("j:translation*");
		while (ni.hasNext()) {
			...
			if (n.isLocked() && n.hasProperty(Constants.JAHIA_LOCKTYPES)) {
				String l = (getSession().isSystem() ? " system " : getSession()
						.getUserID()) + ":" + type;
				Value[] v = n.getProperty(Constants.JAHIA_LOCKTYPES)
						.getValues();
				for (Value value : v) {
					if (value.getString().equals(l)) {
						r.add(LanguageCodeConverters.getLocaleFromCode(n
								.getProperty("jcr:language").getString()));
						break;
					}
				}
	...
	public javax.jcr.lock.Lock getLock() {
		try {
			...
			return new javax.jcr.lock.Lock() {
				...
				public Node getNode() {
					try {
						return getProvider().getNodeWrapper(lock.getNode(),
								getSession());
					} catch (RepositoryException e) {
						logger.warn("Can't get wrapper for node holding lock",
								e);
	...
	public void unlock(String type, String userID)
		...
		if (session.getLocale() != null
				&& !isNodeType(Constants.JAHIANT_TRANSLATION)
				&& hasI18N(session.getLocale(), false)) {
		...
		if (isNodeType(Constants.JAHIANT_TRANSLATION)
				&& !getLockedLocalesForUserAndType(type).isEmpty()) {
			return;
		}
	...
	private void unlock(final Node objectNode, String type, String userID)
			throws RepositoryException {
		if (objectNode.hasProperty("j:locktoken")) {
			Property property = objectNode.getProperty("j:locktoken");
			String token = property.getString();
			Value[] types = objectNode.getProperty(Constants.JAHIA_LOCKTYPES)
					.getValues();
			for (Value value : types) {
				String owner = StringUtils.substringBefore(value.getString(),
						":");
				String currentType = StringUtils.substringAfter(
						value.getString(), ":");
				if (currentType.equals(type)) {
					if (userID.equals(owner)) {
						...
						if (valueList.isEmpty()) {
							objectNode.unlock();
							property.remove();
							objectNode.getProperty(Constants.JAHIA_LOCKTYPES)
									.remove();
						} else {
							objectNode.setProperty(
									Constants.JAHIA_LOCKTYPES,
									valueList.values().toArray(
											new Value[valueList.size()]));
						}
						getSession().save();
					}
				}
	...
	private void clearAllLocks(final Node objectNode)
			throws RepositoryException {
		if (objectNode.hasProperty("j:locktoken")) {
			Property property = objectNode.getProperty("j:locktoken");
			String token = property.getString();
	...
	protected void checkLock() throws RepositoryException {
		if (isLocked() && !session.isSystem()) {
			...
			if (owners.size() == 1 && owners.contains(session.getUserID())) {
				session.addLockToken(objectNode.getProperty("j:locktoken")
						.getString());
			} else {
			...
			if (session.getLocale() != null) {
				...
				if (i18n.isLocked()) {
					...
					if (owners.size() == 1
							&& owners.contains(session.getUserID())) {
						session.addLockToken(i18n.getProperty("j:locktoken")
								.getString());
					} else {
						throw new LockException("Node locked.");
					}
	...
	private List<String> getLockInfos(Node node) throws RepositoryException {
		...
		if (node.hasProperty(Constants.JAHIA_LOCKTYPES)) {
			Value[] values = node.getProperty(Constants.JAHIA_LOCKTYPES)
					.getValues();
			for (Value value : values) {
	...
	public void checkpoint() {
		try {
			JCRObservationManager.doWorkspaceWriteCall(getSession(),
					JCRObservationManager.NODE_CHECKPOINT,
	...
	public List<VersionInfo> getVersionInfos() throws RepositoryException {
		return ServicesRegistry.getInstance().getJCRVersionService()
				.getVersionInfos(session, this);
	}
	 ...
	 * {@inheritDoc}
	 */
	public void orderBefore(String s, String s1)
	 ...
	 * {@inheritDoc}
	 */
	public JCRItemWrapper getPrimaryItem() throws ItemNotFoundException,
			RepositoryException {
	...
	public boolean hasNode(String s) throws RepositoryException {
		...
		if (b
				&& Constants.LIVE_WORKSPACE.equals(getSession().getWorkspace()
						.getName()) && !s.startsWith("j:translation")) {
	 ...
	 * {@inheritDoc}
	 */
	public JCRVersion checkin() throws VersionException,
			UnsupportedRepositoryOperationException, InvalidItemStateException,
			LockException, RepositoryException {
		return (JCRVersion) session.getWorkspace().getVersionManager()
				.checkin(getPath());
	}
	...
	public void doneMerge(Version version) throws VersionException,
		...
		VersionManager versionManager = session.getWorkspace()
				.getVersionManager();
		versionManager.doneMerge(objectNode.getPath(),
				((JCRVersion) version).getRealNode());
	}
	...
	public void cancelMerge(Version version) throws VersionException,
		...
		VersionManager versionManager = session.getWorkspace()
				.getVersionManager();
		versionManager.cancelMerge(objectNode.getPath(),
				((JCRVersion) version).getRealNode());
	}
	...
	public void update(final String srcWorkspace)
		...
		JCRObservationManager.doWorkspaceWriteCall(getSession(),
				JCRObservationManager.NODE_UPDATE, new JCRCallback<Object>() {
					public Object doInJCR(JCRSessionWrapper session)
							throws RepositoryException {
	 ...
	 * {@inheritDoc}
	 */
	public NodeIterator merge(String s, boolean b)
			throws NoSuchWorkspaceException, AccessDeniedException,
	 ...
	 * {@inheritDoc}
	 */
	public String getCorrespondingNodePath(String s)
		...
		if (provider.getMountPoint().equals("/")) {
			...
			return provider.getMountPoint()
					+ objectNode.getCorrespondingNodePath(s);
		}
	}
	...
	public boolean isCheckedOut() throws RepositoryException {
		VersionManager versionManager = session.getProviderSession(provider)
				.getWorkspace().getVersionManager();
		boolean co = versionManager.isCheckedOut(objectNode.getPath());
		if (co && session.getLocale() != null) {
			try {
				co &= versionManager.isCheckedOut(getI18N(session.getLocale())
						.getPath());
			} catch (ItemNotFoundException e) {
				// no i18n node
			}
	 ...
	 * {@inheritDoc}
	 */
	public void restore(String s, boolean b) throws VersionException,
	 ...
	 * {@inheritDoc}
	 */
	public void restore(Version version, boolean b) throws VersionException,
			ItemExistsException, UnsupportedRepositoryOperationException,
			LockException, RepositoryException {
		getRealNode()
				.restore(
						version instanceof JCRVersion ? ((JCRVersion) version).getRealNode()
								: version, b);
	}
	 ...
	 * {@inheritDoc}
	 */
	public void restore(Version version, String s, boolean b)
	 ...
	 * {@inheritDoc}
	 */
	public void restoreByLabel(String s, boolean b) throws VersionException,
			ItemExistsException, UnsupportedRepositoryOperationException,
			LockException, InvalidItemStateException, RepositoryException {
	...
	public VersionHistory getVersionHistory()
		...
		return (VersionHistory) getProvider().getNodeWrapper(
				(Node) getRealNode().getVersionHistory(),
	...
	public Version getBaseVersion()
			throws UnsupportedRepositoryOperationException, RepositoryException {
		return (Version) getProvider().getNodeWrapper(
				(Node) getRealNode().getBaseVersion(),
				(JCRSessionWrapper) getSession());
	}
	 ...
	 * {@inheritDoc}
	 */
	public ExtendedPropertyDefinition getApplicablePropertyDefinition(
		...
		while (iterator.hasNext()) {
			...
			final Map<String, ExtendedPropertyDefinition> definitionMap = type
		...
		if (isNodeType(Constants.JAHIANT_TRANSLATION)
				&& !propertyName.equals("jcr:language")) {
		...
		for (ExtendedNodeType type : types) {
			for (ExtendedPropertyDefinition epd : type
					.getUnstructuredPropertyDefinitions().values()) {
				// check type .. ?
				result = epd;
				applicablePropertyDefinition.put(propertyName, result);
				return result;
	...
	public List<ExtendedPropertyDefinition> getReferenceProperties()
			throws RepositoryException {
		...
		for (ExtendedNodeType type : types) {
			for (ExtendedPropertyDefinition definition : type
					.getUnstructuredPropertyDefinitions().values()) {
				if (definition.getRequiredType() == PropertyType.REFERENCE
						|| definition.getRequiredType() == PropertyType.WEAKREFERENCE) {
					defs.add(definition);
	...
	public ExtendedNodeDefinition getApplicableChildNodeDefinition(
			String childName, String nodeType)
			throws ConstraintViolationException, RepositoryException {
		ExtendedNodeType requiredType = NodeTypeRegistry.getInstance()
		...
		while (iterator.hasNext()) {
			ExtendedNodeType type = iterator.next();
			final Map<String, ExtendedNodeDefinition> definitionMap = type
			...
			if (definitionMap.containsKey(childName)) {
				...
				throw new ConstraintViolationException("Definition type for "
						+ childName + " on node " + getName() + " ("
						+ getPrimaryNodeTypeName() + ") does not match "
						+ nodeType);
			}
			types.add(type);
		}
		for (ExtendedNodeType type : types) {
			for (ExtendedNodeDefinition epd : type
					.getUnstructuredChildNodeDefinitions().values()) {
				for (String req : epd.getRequiredPrimaryTypeNames()) {
					if (requiredType.isNodeType(req)) {
						return epd;
		...
		throw new ConstraintViolationException("Cannot find definition for "
				+ childName + " on node " + getName() + " ("
				...
				if (!fetched) {
					try {
						if (i == 0) {
							...
							next = NodeTypeRegistry.getInstance().getNodeType(
									objectNode.getProperty(
											"jcr:frozenPrimaryType")
											.getString());
						} else {
							if (mix == null) {
								mix = Arrays.asList(getMixinNodeTypes())
										.iterator();
							}
							if (mix.hasNext()) {
								next = mix.next();
	...
	public boolean equals(final Object o) {
		...
		return !(getPath() != null ? !getPath().equals(
				fileNodeWrapper.getPath()) : fileNodeWrapper.getPath() != null);
	}
	 ...
	 *             as long as Jahia doesn't support it
	 */
	public PropertyIterator getProperties(String[] strings)
			throws RepositoryException {
		throw new UnsupportedRepositoryOperationException();
	}
	...
	public PropertyIterator getWeakReferences() throws RepositoryException {
		if (provider.getKey().equals("default")) {
			return new PropertyIteratorImpl(objectNode.getWeakReferences(),
					this);
		} else {
	...
	public PropertyIterator getWeakReferences(String name)
			throws RepositoryException {
		return new PropertyIteratorImpl(objectNode.getWeakReferences(name),
				this);
	}
	...
	public JCRNodeWrapper clone(JCRNodeWrapper sharedNode, String name)
		...
		if (!sharedNode.isNodeType("jmix:shareable")) {
			...
			try {
				final String path = sharedNode
						.getCorrespondingNodePath(Constants.LIVE_WORKSPACE);
				JCRTemplate.getInstance().doExecuteWithSystemSession(null,
						Constants.LIVE_WORKSPACE, new JCRCallback<Object>() {
							public Object doInJCR(JCRSessionWrapper session)
									throws RepositoryException {
		...
		if (getRealNode() instanceof NodeImpl
				&& sharedNode.getRealNode() instanceof NodeImpl) {
			String uri = "";
			if (name.contains(":")) {
			...
			org.apache.jackrabbit.spi.Name jrname = NameFactoryImpl
					.getInstance().create(uri, name);

			NodeImpl node = (NodeImpl) getRealNode();

			try {
				return provider
						.getNodeWrapper(node.clone(
								(NodeImpl) sharedNode.getRealNode(), jrname),
								buildSubnodePath(name), this, session);
			} catch (RepositoryException e) {
				logger.error(e.getMessage(), e);
			}
	...
	public boolean checkValidity() {
		...
		try {
			if (Constants.LIVE_WORKSPACE.equals(jcrSessionWrapper
					.getWorkspace().getName())) {
				if (jcrSessionWrapper.getLocale() != null) {
					if (objectNode.hasProperty("j:published")
							&& !objectNode.getProperty("j:published")
									.getBoolean()) {
						return false;
					}
				}
	...
	public boolean checkLanguageValidity(Set<String> languages) {
		...
		try {
			...
			if (locale != null) {
				...
				if (siteNode != null) {
					Set<String> mandatoryLanguages = siteNode
							.getMandatoryLanguages();
					List<Locale> locales = jcrSessionWrapper.isLive() ? siteNode
							.getActiveLanguagesAsLocales() : siteNode
					...
					for (String mandatoryLanguage : mandatoryLanguages) {
						if (!checkI18nAndMandatoryPropertiesForLocale(LanguageCodeConverters
								.getLocaleFromCode(mandatoryLanguage))) {
							return false;
						}
					}
				}
				...
				if (!b && siteNode != null && siteNode.isMixLanguagesActive()) {
					b = checkI18nAndMandatoryPropertiesForLocale(LanguageCodeConverters
							.getLocaleFromCode(siteNode.getDefaultLanguage()));
				}
				return b;
			} else if (languages != null) {
				for (String language : languages) {
					if (checkI18nAndMandatoryPropertiesForLocale(LanguageCodeConverters
							.getLocaleFromCode(language))) {
						JCRSiteNode siteNode = getResolveSite();
						if (siteNode != null) {
							Set<String> mandatoryLanguages = siteNode
									.getMandatoryLanguages();
							if (mandatoryLanguages == null
									|| mandatoryLanguages.isEmpty()) {
								return true;
							}
							for (String mandatoryLanguage : mandatoryLanguages) {
								if (!checkI18nAndMandatoryPropertiesForLocale(LanguageCodeConverters
	...
	public boolean checkI18nAndMandatoryPropertiesForLocale(Locale locale)
		...
		for (ExtendedPropertyDefinition def : getPrimaryNodeType()
	...
	public JCRSiteNode getResolveSite() throws RepositoryException {
		...
		try {
			...
			if (path.startsWith("/sites/") || path.startsWith("/templateSets/")) {
				...
				try {
					return (site = (JCRSiteNode) (getSession()
							.getNode(index == -1 ? path : path.substring(0,
									index))));
			...
			return (site = (JCRSiteNode) (getSession().getNode(JCRContentUtils
					.getSystemSitePath())));
		} catch (PathNotFoundException e) {
		} catch (ItemNotFoundException e) {
		}
		return null;
		// return
		// ServicesRegistry.getInstance().getJahiaSitesService().getDefaultSite();
	}

	public PropertyIterator getExternalWeakReferences()
			throws RepositoryException {
		QueryManager queryManager = session.getWorkspace().getQueryManager();
		Query query = queryManager.createQuery(
				"SELECT * FROM [jmix:externalReference] as n WHERE n.["
						+ REFERENCE_NODE_IDENTIFIERS_PROPERTYNAME + "]='"
						+ getIdentifier() + "'", Query.JCR_SQL2);
			...
			if (foundPropertyName != null) {
				...
				if (epd != null) {
					Property nodeProperty = new ExternalReferencePropertyImpl(
							foundPropertyName, node, session, getIdentifier(),
							this);
					matchingProperties.add(new JCRPropertyWrapperImpl(this,
		...
		try {
			...
			try {
				...
				if (itemName != null) {
					...
					title = new TextExtractor(new Source(s != null ? s
							: getName())).toString();
				}
			} catch (RepositoryException e1) {
				title = null;
