...
public class DaylyTimeBasedPublishingNode extends NodeImpl {
    ...
    public DaylyTimeBasedPublishingNode(SessionImpl session, DayInWeekBean dayInWeekBean, JahiaContentNodeImpl node) {
        super(session);
        try {
            setDefinition(NodeTypeRegistry.getInstance().getNodeType("jmix:hourlyTimebasedPublished").getChildNodeDefinitionsAsMap().get("j:acl"));
            setNodetype(NodeTypeRegistry.getInstance().getNodeType("jnt:hourlyTimebasedPublished"));
