...
class OrphanedVersionHistoryChecker {
    ...
    private static final Name NAME_NT_VERSION_HISTORY = NameFactoryImpl.getInstance().create(Name.NS_NT_URI,
            "versionHistory");
    ...
    private final OrhpanedVersionHistoryCheckStatus status;

    OrphanedVersionHistoryChecker(OrhpanedVersionHistoryCheckStatus status, long maxOrphans, boolean deleteOrphans,
            OutWrapper out) {
        super();
        this.status = status;
    ...
    private void delete(JCRSessionWrapper session) {
        ...
        try {
            ...
            out.echo("deleted {} version histories", (status.deleted - nb));
        } catch (Exception e) {
