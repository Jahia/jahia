...
public class JahiaHighlighter {
    ...
    public static String highlight(TermPositionVector tvec,
                                   ...
                                   int surround)
            throws IOException {
        return new JahiaHighlighter().doHighlight(tvec, queryTerms, text,
    ...
    protected String mergeFragments(TermVectorOffsetInfo[] offsets,
        ...
        for (int i = 0; i < bestFragmentsList.size(); i++) {
            ...
            if (skip >= surround) {
// AGDR-81: fix position of highlighted text to not start with the highlighted term, but use some
// surrounding context at the beginning                
//           if (i > 0) {
                    skip -= surround;
//                }
                // skip
