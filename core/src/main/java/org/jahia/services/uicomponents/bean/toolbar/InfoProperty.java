 ...
 * Time: 11:10:32
 */
public class InfoProperty extends Property {
    ...
    public String getRealValue(JCRSiteNode site, JahiaUser user, Locale locale) {
        if (key.equalsIgnoreCase(ORG_JAHIA_TOOLBAR_ITEM_CONNECTED_USER)) {
            return user.getName();
        } else if (key.startsWith(ORG_JAHIA_TOOLBAR_ITEM_LICENSE_INFO)) {
            try {

                long warningdays = 10;
                try {
                    if (key.indexOf("_") > 0) {
                        warningdays = Long.parseLong(key.substring(key.indexOf("_") + 1, key.length()));
                    }
                ...
                if (expirationTime > 0 && (now >= expirationTime || (expirationTime - now) <= 1000L * 60 * 60 * 24 * warningdays)) {
                    return JahiaResourceBundle.getJahiaInternalResource(
                            "jahia.toolbar.license.expire", locale)
                            + " "
