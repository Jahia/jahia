...
public class AclNameEditor {
    ...
    public static void initACLNameEditors() {
        ...
        for (int i = 0; i < Integer.MAX_VALUE; i++) {
            ...
            if (panel != null) {
                String heightAttribute = DOM.getElementAttribute(panel
                        .getElement(), "height");
                final String height = heightAttribute.length() > 0 ? heightAttribute
                        : "200px";
                final int aclId = Integer.parseInt(DOM.getElementAttribute(
                        panel.getElement(), "aclid"));
                ACLService.App.getInstance().getACL(aclId,
                        new AsyncCallback<GWTJahiaNodeACL>() {
                            public void onFailure(Throwable caught) {
                                Log.error("ACL load failed for the ID '"
                                        + aclId + "'", caught);
                            }

                            public void onSuccess(GWTJahiaNodeACL result) {
                                ContentPanel aclPanel = new AclNameEditor(
                                        result, DOM.getElementAttribute(panel
                                                .getElement(), "fieldid"),
                                        "true".equals(DOM.getElementAttribute(panel
                                                .getElement(), "readonly")))
                                        .renderNewAclPanel();
    ...
    public ContentPanel renderNewAclPanel() {
        ...
        col.setRenderer(new CellRenderer<TableItem>() {
            public String render(final TableItem item, final String property,
                    final Object value) {
