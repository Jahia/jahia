...
public class StateDisplay extends FlexTable {
    ...
    public StateDisplay(final GWTJahiaPageContext page,
            final GWTJahiaGlobalState globalState) {
        super();
        if (globalState != null) {
            int row = getRowCount();
            ...
            if (globalState.hasWorkflowState()) {
                setWidget(0, row++, new StateIcon(globalState
                        ...
                        + globalState.getWorkflowState()
                                .getExtendedWorkflowState(), true));
            }
            if (globalState.hasTimebasedPublishingState()) {
                setWidget(0, row++, new TimebasedPublishingIcon(globalState
                        .getTimebasedPublishingState(), page));
            }
            if (globalState.hasAclDiffState()) {
                setWidget(0, row++, new AclDiffIcon(globalState
