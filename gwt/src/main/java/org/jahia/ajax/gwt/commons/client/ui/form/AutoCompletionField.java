...
import org.jahia.ajax.gwt.commons.client.ui.menu.AutoCompletionMenu;
import org.jahia.ajax.gwt.commons.client.beans.ValueDisplayBean;
...
public class AutoCompletionField extends TriggerField<String> {
    ...
    public void updateChoices(String match) {
        ...
        choices.removeAll();
        final ListStore<ValueDisplayBean> datas = completionItems.getCompletionItems(jahiaPage,match+"*");
        ...
        if (datas != null && datas.getLoader() != null) {
            ...
            datas.addStoreListener(new StoreListener(){
                public void storeDataChanged(StoreEvent event){
                    Iterator<ValueDisplayBean> it = datas.getModels().iterator();
                    ValueDisplayBean data;
                    while (it.hasNext()){
                        data = it.next();
