...
package org.jahia.ajax.gwt.commons.client.ui.rss;
...
import org.jahia.ajax.gwt.commons.client.beans.rss.GWTRSSEntry;
...
public class RSSWidget extends ContentPanel {
    ...
    private void initWidget(){
        ...
        this.view = new ListView<GWTRSSEntry>() {
            @Override
            protected GWTRSSEntry prepareData(GWTRSSEntry model) {
        ...
        final RpcProxy proxy = new RpcProxy<PagingLoadConfig, PagingLoadResult<GWTRSSEntry>>() {
            @Override
            public void load(PagingLoadConfig pageConfig, AsyncCallback<PagingLoadResult<GWTRSSEntry>> callback) {
                ...
                if(gwtrssFeed != null && gwtrssFeed.getEntries() != null){
                    List<GWTRSSEntry> entries = new ArrayList<GWTRSSEntry>();
                    int offset = pageConfig.getOffset();
                    ...
                    callback.onSuccess(new  BasePagingLoadResult<GWTRSSEntry>(entries,
                            pageConfig.getOffset(),gwtrssFeed.getEntries().size()));
                    view.refresh();
                } else {
                    callback.onSuccess(new  BasePagingLoadResult<GWTRSSEntry>(new ArrayList<GWTRSSEntry>()));
                    view.refresh();
                }
            }
        };
    ...
    public GWTRSSFeed getGwtrssFeed() {
        return gwtrssFeed;
    }

    public void setGwtrssFeed(GWTRSSFeed gwtrssFeed) {
    ...
    public void execute() {
        ...
        service.loadRssFeed(page,this.feedURL,this.maxEntries,
            new AsyncCallback<GWTRSSFeed>() {
                public void onSuccess(GWTRSSFeed o) {
                    gwtrssFeed = o;
                    refreshData(false);
                }
