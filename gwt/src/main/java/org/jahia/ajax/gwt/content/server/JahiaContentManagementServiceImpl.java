...
public class JahiaContentManagementServiceImpl extends JahiaRemoteService implements JahiaContentManagementService {
    private static final Logger logger = LoggerFactory.getLogger(JahiaContentManagementServiceImpl.class);
    ...
    public void setToolbarWarningsService(ToolbarWarningsService GWTToolbarWarningsService) {
        this.toolbarWarningsService = GWTToolbarWarningsService;
    }
...
// --------------------- Interface JahiaContentManagementServiceAsync ---------------------
    ...
    public Map<String, List<? extends ModelData>> getNodesAndTypes(List<ModelData> getNodesParams, List<String> types) throws GWTJahiaServiceException {
        ...
        for (ModelData params : getNodesParams) {
            if (logger.isDebugEnabled()) {
                paths.addAll(params.get("paths"));
            }
            nodes.addAll(getNodesInternal(params.get("paths"), params.get("fields")));
        }
        m.put("nodes", nodes);
...
//        setLock(Arrays.asList(node.getPath()), false);
            ...
            if (node.get(GWTJahiaNode.INCLUDE_CHILDREN) != null) {
                ...
                for (ModelData modelData : node.getChildren()) {
                    ...
                    String renamedFrom = subNode.get("renamedFrom");
                    ...
                    if (subNode.get("nodeLangCodeProperties") != null && subNode.get("nodeProperties") != null) {
                        if (nodeWrapper.hasNode(JCRContentUtils.escapeLocalNodeName(subNode.getName()))) {
                            saveNode(subNode, null, subNode.get("nodeLangCodeProperties"), subNode.get("nodeProperties"), new HashSet<String>());
                        } else {
            ...
            if (node.get("vanityMappings") != null) {
                saveUrlMappings(node, node.get("vanityMappings"));
            }
            if (node.get("visibilityConditions") != null) {
                List<GWTJahiaNode> visibilityConditions = node.get("visibilityConditions");
                ...
                try {
                    ...
                    if (Boolean.TRUE.equals(node.get("conditions-published"))) {
                        publication.publish(Collections.singletonList(jcrSessionWrapper.getNode(node.getPath() + "/" + VisibilityService.NODE_NAME).getIdentifier()));
                    }
                } catch (RepositoryException e) {
            ...
            if (node.get("activeWorkflows") != null) {
                workflow.updateWorkflowRules(node,
                        node.get("activeWorkflows"), jcrSessionWrapper);
            }
    ...
    private void saveProperties(GWTJahiaNode node, Map<String, List<GWTJahiaNodeProperty>> langCodeProperties, List<GWTJahiaNodeProperty> sharedProperties, Set<String> removedTypes) throws GWTJahiaServiceException {
        ...
        try {
            ...
            // save shared properties
            properties.saveProperties(Collections.singletonList(node), sharedProperties, removedTypes, session, getUILocale(), getSession().getId());
            ...
            while (langCode.hasNext()) {
                ...
                sessions.add(session);
                properties.saveProperties(Collections.singletonList(node), props, removedTypes, session, getUILocale(), getSession().getId());
            }
            for (JCRSessionWrapper sessionWrapper : sessions) {
                sessionWrapper.validate();
            }
    ...
    public GWTJahiaNode createNode(String parentPath, GWTJahiaNode newNode) throws GWTJahiaServiceException {
        ...
        return createNode(parentPath, newNode.getName(), newNode.getNodeTypes().get(0), newNode.getNodeTypes().subList(1, newNode.getNodeTypes().size()), newNode.get("newAcl"),
                newNode.get("nodeProperties"), newNode.get("nodeLangCodeProperties"), nodes, null, true);
    }
    ...
    public SessionValidationResult isValidSession() throws GWTJahiaServiceException {
        ...
        if (session != null) {
            ...
            if (date != null && (date / 1000 == lastAccessed / 1000)) {
                ...
                if (now - first < session.getMaxInactiveInterval() * 1000L) {
    ...
    public BasePagingLoadResult<GWTJahiaJobDetail> getJobs(int offset, int limit, String sortField, String sortDir,
                                                           String groupBy, List<String> groupNames) throws GWTJahiaServiceException {
        List<GWTJahiaJobDetail> jobList = schedulerHelper.getAllJobs(new HashSet<>(groupNames), sortField, sortDir, groupBy);
        int size = jobList.size();
        jobList = new ArrayList<>(jobList.subList(offset, Math.min(size, offset + limit)));
    ...
    public ModelData getVisibilityInformation(String path) throws GWTJahiaServiceException {
        ...
        try {
            ...
            for (Map.Entry<JCRNodeWrapper, Boolean> entry : conditionMatchesDetails.entrySet()) {
                List<String> fields = new ArrayList<String>(Collections.singletonList(GWTJahiaNode.PUBLICATION_INFO));
