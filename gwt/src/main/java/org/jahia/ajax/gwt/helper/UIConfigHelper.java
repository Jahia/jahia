...
public class UIConfigHelper {
     ...
     */
    public org.jahia.ajax.gwt.client.data.toolbar.GWTManagerConfiguration getGWTManagerConfiguration(JCRNodeWrapper contextNode, JCRSiteNode site, JahiaUser jahiaUser, Locale locale, Locale uiLocale, HttpServletRequest request, String name) throws GWTJahiaServiceException {
        try {
            ManagerConfiguration config = (ManagerConfiguration) getThemedConfiguration(name, request);
            if (config != null) {
                logger.debug("Config. " + name + " found.");
                org.jahia.ajax.gwt.client.data.toolbar.GWTManagerConfiguration gwtConfig = new org.jahia.ajax.gwt.client.data.toolbar.GWTManagerConfiguration();
