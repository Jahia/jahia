...
public class WorkflowHelper {
    private static final Logger logger = org.slf4j.LoggerFactory.getLogger(WorkflowHelper.class);
    ...
    public void startWorkflow(String path, GWTJahiaWorkflowDefinition def, JCRSessionWrapper session,
        ...
        try {
            ...
            service.startProcessAsJob(Collections.singletonList(node.getIdentifier()), session, def.getId(), def.getProvider(), map, comments);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
    ...
    public void startWorkflow(List<String> uuids, GWTJahiaWorkflowDefinition def, JCRSessionWrapper session,
        ...
        try {
            Map<String, Object> map = getVariablesMap(properties, def.getFormResourceName());
            map.putAll(args);
            // Convert GWT Publication infos to Publication infos and replace it in the map
            if (args.containsKey("customWorkflowInfo")) {
                PublicationWorkflow publicationWorkflow = (PublicationWorkflow) args.get("customWorkflowInfo");
                List<ProcessedPublicationInfo> publicationInfos = new ArrayList<>();
                for (GWTJahiaPublicationInfo gwtInfo : publicationWorkflow.getPublicationInfos()) {
                    ProcessedPublicationInfo info = new ProcessedPublicationInfo();
                    info.setStatus(gwtInfo.getStatus());
                    info.setPath(gwtInfo.getPath());
                    info.setNodeType(gwtInfo.getNodetype());
                    info.setTitle(gwtInfo.getTitle());
                    info.setUuid(gwtInfo.getUuid());
                    info.setWorkflowDefinition(gwtInfo.getWorkflowDefinition());
                    info.setMainPath(gwtInfo.getMainPath());
                    info.setLanguage(gwtInfo.getLanguage());
                    info.setLocked(gwtInfo.isLocked());
                    info.setWorkInProgress(gwtInfo.isWorkInProgress());
                    info.setIsAllowedToPublishWithoutWorkflow(gwtInfo.isAllowedToPublishWithoutWorkflow());
                    info.setIsNonRootMarkedForDeletion(gwtInfo.isNonRootMarkedForDeletion());
                    info.setI18nUuid(gwtInfo.getI18nUuid());
                    info.setDeletedI18nUuid(gwtInfo.getDeletedI18nUuid());
                    info.setMainPathIndex(gwtInfo.getMainPathIndex());
                    info.setWorkflowGroup(gwtInfo.getWorkflowGroup());
                    info.setWorkflowTitle(gwtInfo.getWorkflowTitle());
                    info.setMainUUID(gwtInfo.getMainUUID());
                    publicationInfos.add(info);
                }
                map.put("processedWorkflowInfo", new ProcessedPublicationWorkflow(publicationInfos));
            }
                ...
                if (gwtWfHistory == null) {
                    ...
                    for (String nodeId : nodeIds) {
                        try {
                            ...
                            gwtWfHistory.set("nodeWrapper", SpringContextSingleton.getInstance().getContext().getBeansOfType(NavigationHelper.class).values().iterator().next().getGWTJahiaNode(nodeWrapper));
