...
package org.jahia.ajax.gwt.templates.components.mysettings.client.ui;
...
import org.jahia.ajax.gwt.commons.client.beans.BasicDataBean;
...
import org.jahia.ajax.gwt.commons.client.beans.GWTJahiaUserProperty;
...
public class MySettingsPanel extends AbsolutePanel {
        ...
        if (userPorpeties != null) {
            for (GWTJahiaUserProperty gwtJahiaUserProperty : userPorpeties) {
                if ("checkbox".equals(gwtJahiaUserProperty.getDisplay())) {
                    ...
                    final BasicDataBean data = gwtJahiaUserProperty.getValue();
                    ...
                    final ListStore<BasicDataBean> store = new ListStore<BasicDataBean>();
                    store.add(gwtJahiaUserProperty.getValues());

                    final ComboBox<BasicDataBean> select = new ComboBox<BasicDataBean>();
                    select.setDisplayField(BasicDataBean.DISPLAY_NAME);
                ...
                if (pwd.equalsIgnoreCase(confirmPwd)) {
                    ...
                    for (Field field : formPanel.getFields()) {
                        ...
                        if (field instanceof ComboBox) {
                            jahiaUserProperty.setValue(((ComboBox<BasicDataBean>)field).getValue());
                        } else {
                            jahiaUserProperty.setValue(new BasicDataBean(field.getRawValue(), null));
                        }
                    ...
                    JahiaContentService.App.getInstance().updateJahiaUserProperties(newJahiaUserProperties, removeJahiaUserProperties, new AsyncCallback() {
                        ...
                        public void onSuccess(Object o) {
                            ...
                            GWTAjaxActionResult gwtAjaxActionResult = (GWTAjaxActionResult) o;
     ...
     */
    private void handleAjaxActionResult(GWTAjaxActionResult gwtAjaxActionResult) {
        if (gwtAjaxActionResult != null) {
            List<GWTAjaxActionResultError> gwtAjaxActionResultErrors = gwtAjaxActionResult.getErrors();
            if (gwtAjaxActionResultErrors != null && !gwtAjaxActionResultErrors.isEmpty()) {
                // display errors
                String errors = null;
                for (GWTAjaxActionResultError error : gwtAjaxActionResultErrors) {
                    if (errors == null) {
