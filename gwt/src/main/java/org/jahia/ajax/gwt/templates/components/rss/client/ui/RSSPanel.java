...
package org.jahia.ajax.gwt.templates.components.rss.client.ui;
...
public class RSSPanel extends ContentPanel {
        ...
        if (gwtrssFeed != null) {
            ...
            if (gwtrssFeed.getEntries() != null) {
                ...
                for (int i = getFeedEntryIndex(); (i < gwtrssFeed.getEntries().size() && row < getEntriesPerPage()); i++) {
                    GWTRSSEntry entry = gwtrssFeed.getEntries().get(i);
     ...
     * Load RSS entries
     */
    private class RssLoadAsyncCallback implements AsyncCallback<GWTRSSFeed> {
