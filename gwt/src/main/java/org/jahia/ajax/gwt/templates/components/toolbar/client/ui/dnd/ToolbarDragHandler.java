    ...
    private void updateToolbarState(DragContext dragContext, Widget nextParent) {
        if (dragContext.draggable instanceof JahiaToolbar) {
            ...
            if (nextParent instanceof TargetVerticalPanel) {
                ...
                for (int i = 0; i < targetVerticalPanel.getWidgetCount(); i++) {
                    ...
                    if (w instanceof JahiaToolbar) {
                        ...
                        if (targetVerticalPanel.isTop()) {
                            jahiaToolbar.getGwtToolbar().getState().setValue(Constants.TOOLBAR_TOP);
                        } else if (targetVerticalPanel.isRigth()) {
                            jahiaToolbar.getGwtToolbar().getState().setValue(Constants.TOOLBAR_RIGHT);
                        }
                ...
                if (previousState != Constants.TOOLBAR_RIGHT && previousState != Constants.TOOLBAR_VERTICAL_BOX) {
                    draggableJahiaToolbar.getGwtToolbar().getState().setValue(Constants.TOOLBAR_HORIZONTAL_BOX);
                } else {
                    draggableJahiaToolbar.getGwtToolbar().getState().setValue(Constants.TOOLBAR_VERTICAL_BOX);
                }
