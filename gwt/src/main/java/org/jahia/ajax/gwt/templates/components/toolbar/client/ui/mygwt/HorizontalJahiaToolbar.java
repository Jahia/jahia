...
import org.jahia.ajax.gwt.templates.components.toolbar.client.ui.mygwt.provider.JahiaToolItemProvider;
...
public class HorizontalJahiaToolbar extends JahiaToolbar {
    ...
    public HorizontalJahiaToolbar(GWTToolbar gwtToolbar) {
        super(gwtToolbar);
    }

    public HorizontalJahiaToolbar(ToolbarManager toolbarManager, GWTToolbar gwtToolbar) {
    ...
    public void createToolBarUI() {
        if (!loaded) {
            ...
            // add items
            List<GWTToolbarItemsGroup> itemsGroupList = gwtToolbar.getGwtToolbarItemsGroups();
            Log.debug("---- Nb items groupe: " + itemsGroupList.size());
            for (int i = 0; i < itemsGroupList.size(); i++) {
                GWTToolbarItemsGroup gwtToolbarItemsGroup = itemsGroupList.get(i);
                ...
                if (gwtToolbarItemsGroup.getType() != null && gwtToolbarItemsGroup.getType().equalsIgnoreCase(Constants.ITEMSGROUP_FILL)) {
                    ...
                    if (i != itemsGroupList.size() - 1 && itemsGroupList.get(i + 1).isNeedSeparator()) {
                        // add separator
                        if (gwtToolbarItemsGroup.getLayout() != Constants.ITEMSGROUP_TABS) {
    ...
    public void fillToolBar(final GWTToolbarItemsGroup gwtToolbarItemsGroup, ToolBar toolBar, TabPanel tabPanel) {
        ...
        for (int i = 0; i < toolbarItemsGroupList.size(); i++) {
            // add items
            GWTToolbarItem gwtToolbarItem = (GWTToolbarItem) toolbarItemsGroupList.get(i);
            ...
            if (toolbarItemWidgetProvider == null && !isSeparator(gwtToolbarItem)) {
                ...
                if (gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_MENU || gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_MENU_RADIO || gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_MENU_CHECKBOX) {
                ...
                } else if (gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_SELECT) {
                    // handle case of menuSeparator
                    if (isSeparator(gwtToolbarItem)) {
                        // add menu separator only if we have at least one menuitem
                        if (menu.getItemCount() > 0) {
                ...
                } else if (gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_BUTTON || gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_LABEL || gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_BUTTON_LABEL) {
                    if (isSeparator(gwtToolbarItem)) {
                        toolBar.add(new SeparatorToolItem());
                    } else {
                ...
                } else if (gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_TABS) {
                    if (!isSeparator(gwtToolbarItem)) {
                    ...
                    gwtToolbarItemsGroup.setLayout(Constants.ITEMSGROUP_BUTTON);
                    if (isSeparator(gwtToolbarItem)) {
                        toolBar.add(new SeparatorToolItem());
                    } else {
                        ToolItem toolbarItem = toolbarItemWidgetProvider.createToolItem(gwtToolbarItemsGroup, gwtToolbarItem);
                ...
                if (gwtToolbarItemsGroup.getLayout() == Constants.ITEMSGROUP_BUTTON) {
