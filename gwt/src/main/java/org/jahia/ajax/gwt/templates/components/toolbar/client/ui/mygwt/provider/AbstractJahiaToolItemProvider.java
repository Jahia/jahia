...
public abstract class AbstractJahiaToolItemProvider extends JahiaToolItemProvider {
    public Widget createWidget(final GWTToolbarItemsGroup gwtToolbarItemsGroup, final GWTToolbarItem gwtToolbarItem) {
    ...
    public ToolItem createToolItem(final GWTToolbarItemsGroup gwtToolbarItemsGroup, final GWTToolbarItem gwtToolbarItem) {
        ...
        if (toolbarItem instanceof TextToolItem) {
            if (layout == Constants.ITEMSGROUP_BUTTON_LABEL || layout == Constants.ITEMSGROUP_LABEL) {
                if (gwtToolbarItem.isDisplayTitle()) {
                    ((TextToolItem) toolbarItem).setText(gwtToolbarItem.getTitle());
                }
            }
            if (layout == Constants.ITEMSGROUP_BUTTON_LABEL) {
                ((TextToolItem) toolbarItem).setIconStyle(gwtToolbarItem.getMinIconStyle());
            }
            if (layout == Constants.ITEMSGROUP_BUTTON) {
    ...
    public MenuItem createMenuItem(final GWTToolbarItemsGroup gwtToolbarItemsGroup, final GWTToolbarItem gwtToolbarItem) {
        ...
        int layout = gwtToolbarItemsGroup.getLayout();
        if (layout == Constants.ITEMSGROUP_MENU) {
            menuItem = new MenuItem();
            menuItem.setIconStyle(gwtToolbarItem.getMinIconStyle());
        } else if (layout == Constants.ITEMSGROUP_MENU_RADIO) {
        ...
        } else if (layout == Constants.ITEMSGROUP_MENU_CHECKBOX) {
            menuItem = new CheckMenuItem();
            ((CheckMenuItem) menuItem).setChecked(gwtToolbarItem.isSelected());
        } else {
            menuItem = new MenuItem();
    ...
    public DataListItem createDataListItem(final DataList list, final GWTToolbarItemsGroup gwtToolbarItemsGroup, final GWTToolbarItem gwtToolbarItem) {
        ...
        int layout = gwtToolbarItemsGroup.getLayout();
        if (layout != Constants.ITEMSGROUP_MENU_RADIO && layout != Constants.ITEMSGROUP_MENU_CHECKBOX) {
     ...
     * @return
     */
    public TabItem createTabItem(final TabPanel tabPanel, final GWTToolbarItemsGroup gwtToolbarItemsGroup, final GWTToolbarItem gwtToolbarItem) {
     ...
     * @return
     */
    public abstract SelectionListener<ComponentEvent> getSelectListener(final GWTToolbarItem gwtToolbarItem);
    ...
    public abstract ToolItem createNewToolItem(final GWTToolbarItem gwtToolbarItem);
