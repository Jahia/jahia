...
<c:if test="${createdBy.string ne ' guest '}">
    <c:set var="userNode" value="${user:lookupUser(createdBy.string)}"/>
...
<li class="genericListCommentLi">
    ...
    <p>
        <span class="author">
            <c:if test="${createdBy.string ne 'guest'}">
                <a href="<c:url value='${url.base}${user:lookupUser(createdBy.string).path}.html'/>">${createdBy.string}</a></c:if>
