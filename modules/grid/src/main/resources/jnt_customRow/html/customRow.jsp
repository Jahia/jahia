...
<c:if test="${renderContext.editModeConfigName eq 'studiomode'}">
    <%--<script type="text/javascript">--%>
        ...
        <%--onGWTFrameLoad(function () {--%>
            <%--&lt;%&ndash;<c:forEach items="${colMap}" var="col" varStatus="count">&ndash;%&gt;--%>
            <%--&lt;%&ndash;if ($("#grid_${fn:replace(currentNode.identifier,'-','_')}_${count.count} > div").size() == 1) {&ndash;%&gt;--%>
            <%--&lt;%&ndash;$("#grid_${fn:replace(currentNode.identifier,'-','_')}_${count.count}").append($('<div class="${renderContext.editModeConfigName}area">' +&ndash;%&gt;--%>
            <%--&lt;%&ndash;'<div class="${renderContext.editModeConfigName}areaTemplate">'+&ndash;%&gt;--%>
            <%--&lt;%&ndash;'<span>Empty content : ${currentNode.name}</span></div></div>'));&ndash;%&gt;--%>
            <%--&lt;%&ndash;}&ndash;%&gt;--%>
