...
--%><%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %><c:set target="${renderContext}" property="contentType" value="application/xml"/><?xml version="1.0" encoding="UTF-8"?>
...
<c:url var="searchUrl" value="${url.server}${url.base}${renderContext.site.path}/search-results.html?src_terms%5b0%5d.term={searchTerms}">
    ...
    <c:param name="src_languages.values" value="${currentResource.locale}"/>
</c:url>
<c:set var="title" value="${functions:default(currentNode.propertiesAsString['jcr:title'], 'Jahia OpenSearch')}"/>
<c:set var="description" value="${functions:default(currentNode.propertiesAsString['jcr:description'], 'Search Jahia content using OpenSearch')}"/>
...
<OpenSearchDescription xmlns="http://a9.com/-/spec/opensearch/1.1/">
    ...
    <Url type="text/html" method="get" template="${fn:escapeXml(searchUrl)}"/>
