...
<h2>
    <fmt:message key="siteSettings.manageSiteRoles" />
</h2>
...
<script type="text/javascript">
    ...
    function revokeRole(principal,role) {
        if(confirm('<fmt:message key="siteSettings.manageSiteRoles.revokeRole.confirm" />')) {
            $('#role').val(role);
...
<form action="${flowExecutionUrl}" method="POST" id="roleForm">
    ...
    <c:forEach items="${siteRoles}" var="entry" >
