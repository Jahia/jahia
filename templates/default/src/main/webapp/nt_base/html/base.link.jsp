...
<c:if test="${not omitFormatting && not empty param.cssClass}"><c:set var="class">${param.cssClass}</c:set></c:if>
...
<span class="icon ${class}"></span><a href="${urlValue}"${linkTitle}>${fn:escapeXml(not param.useNodeNameAsTitle && not empty title.string ? title.string : currentNode.name)}</a>