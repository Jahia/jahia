...
<%--@elvariable id="url" type="org.jahia.services.render.URLGenerator"--%>
	...
	<c:if test="${mode.string eq 'site'}">
		<jcr:sql var="result" sql="SELECT * FROM [${type.string}] AS page WHERE ISDESCENDANTNODE(page,'${renderContext.site.path}') ORDER BY [jcr:${criteria.string}] DESC" limit="${nbOfResult.long}" />
	</c:if>
	<c:if test="${mode.string eq 'platform'}">
		<jcr:sql var="result" sql="SELECT * FROM [${type.string}] AS page WHERE ISDESCENDANTNODE(page,'${renderContext.site.parent.path}') ORDER BY [jcr:${criteria.string}] DESC" limit="${nbOfResult.long}" />
	</c:if>
