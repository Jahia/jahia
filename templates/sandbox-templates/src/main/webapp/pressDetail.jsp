                    ...
                    <div id="wrapper"><!--start wrapper-->
                        <div id="content2"><!--start content-->
                            <div class="spaceContent"><!--start spaceContent -->
                                    <%-- Retreive container detail --%>
                                <template:getContainer containerID="${param.id}" valueID="pressReleaseContainer"/>
                                <template:getContainerField fieldName="pressReleaseTitle"
                                                            containerBean="${pressReleaseContainer}"
                                                            valueID="pressReleaseTitle"/>
                                <template:getContainerField fieldName="pressReleaseBody"
                                                            containerBean="${pressReleaseContainer}"
                                                            valueID="pressReleaseBody"/>
                                <template:include page="common/breadcrumb.jsp"/>
                                <div class="box">
                                    <h2><c:out value="${pressReleaseTitle.value}"/></h2>

                                    <p>
                                        <c:out value="${pressReleaseBody.value}"/>
                                    </p>
                                    <ui:actionMenu contentObjectName="pressReleaseContainer"
                                                   namePostFix="pressRelease" labelKey="pressRelease.update"/>
                                </div>
                            </div>
                            <!--stop space content-->
                        </div>
