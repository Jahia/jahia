...
<c:set var="startLevel"
       value="${not empty param.startLevel ? param.startLevel : currentNode.properties['j:startLevel'].long}"/>
<c:set var="useVisitedNodeAsBasline"
       value="${not empty param.useVisitedNodeAsBaseline ? param.useVisitedNodeAsBaseline : currentNode.properties['j:useVisitedNodeAsBaseline'].boolean}"/>
<c:set var="mainNode"
       value="${not empty baselineNode ? baselineNode.node : (useVisitedNodeAsBaseline or jcr:isNodeType(renderContext.mainResource.node, 'jnt:page') ? renderContext.mainResource.node : jcr:getParentOfType(renderContext.mainResource.node, 'jnt:page'))}"/>
...
<c:forEach items="${menu}" var="navMenuBean" varStatus="status">
    ...
    <c:if test="${navMenuBean.lastInLevel and not empty navMenuBean.parentItem}">
        </ul>
        </div>
    </c:if>
</c:forEach>
