...
<template:containerList name="lastNewsContainer" id="lastNews"
    ...
    <template:container id="lastNewsContainer" cache="off" actionMenuNamePostFix="lastnews"
        ...
        <template:getContentObjectCategories valueID="newsContainerCatKeys"
                                         objectKey="contentContainer_${pageScope.lastNewsContainer.ID}"/>
        <template:field name="maxNews" valueBeanID="maxNews" display="false" defaultValue="10"/>
        <template:containerList windowSize="${maxNews}" id="newsList" displayActionMenu="false">
            <query:containerQuery>
                <query:selector nodeTypeName="web_templates:newsContainer" selectorName="newsList"/>
                <c:if test="${!empty newsContainerCatKeys}">
                    <query:equalTo propertyName="${queryConstants.CATEGORY_LINKS}" value="${newsContainerCatKeys}"
            ...
            <%@ include file="../../../containers/newsContent/newsDisplay.jspf" %>
