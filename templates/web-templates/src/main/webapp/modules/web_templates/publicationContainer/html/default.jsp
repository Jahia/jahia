        ...
        <div class="publicationListSpace"><!--start publicationListSpace -->
            <div class="publicationPhoto">
                <a href="${filePublication.file.downloadUrl}">
                    ...
                    <c:if test="${!previewsPublication.file.image}"><img
                            src="<utility:resolvePath value='theme/${requestScope.currentTheme}/img/no_preview.png'/>"
                            alt="no preview"/></c:if>
                </a>
            </div>
            ...
            <div class="publicationBody"><!--start publicationBody -->
                ...
                <p class="publicationAuthor"><c:if test="${!empty author}"><fmt:message
                        key="web_templates_publicationContainer.author"/>: ${authorPublication.string}</c:if></p>

                <p class="publicationSource"><c:if test="${!empty source}"><fmt:message
                        key="web_templates_publicationContainer.source"/>: ${authorPublication.source}</c:if></p>
                ...
                <div class="publicationAction">
                    ...
                    <fmt:formatNumber var="num" pattern="### ### ###.##" type="number"
                                      value="${(filePublication.file.size/1024)}"/>
                    <a class="publicationDownload" href="${filePublication.file.downloadUrl}"><fmt:message
