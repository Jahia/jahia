		...
		mutationObserverCallback = function(mutation){
			...
			if(mutation.addedNodes.length > 0){
				...
				if($(mutation.addedNodes[0]).hasClass("x-grid3-row")){
					...
					for(treeID in queue["treechange"]){
			...
			//
			// }
			// if(publicationStatus.attr("src") && (publicationStatus.attr("src") !== data.publication)){
			// 	data.publication = publicationStatus.attr("src");
			//
			// 	publishSplit = data.publication.split("/");
			// 	publishName = publishSplit[publishSplit.length - 1];
			// 	friendlyPublishName = publishName.substr(0, publishName.length - 4);
			//
			// 	eventHandlers.publicationStatusChanged(friendlyPublishName);
			//
			// }
			// /* END MESSY */
...
(function(exposeAs){
	...
	var eventListeners = {
		attach: function(){
			...
			Dex("body > div:nth-child(1) > div:nth-child(1)").onAttr("class", eventHandlers.changedMode);
