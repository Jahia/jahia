<%@ page contentType="text/xml;charset=UTF-8" %><%response.setHeader("Cache-Control", "no-cache, no-store, must-revalidate, proxy-revalidate, max-age=0"); response.setHeader("Pragma", "No-Cache"); response.setDateHeader("Expires", 295075800000L); %><?xml version="1.0" encoding="UTF-8"?>
<%--
...
<c:set var="title" value="${h:default(param.title, 'Jahia document repository')}"/>
    ...
    <c:url var="url" value="${currentSite.externalUrl}?src_mode=files&src_terms[0].fields.content=true&src_terms[0].term={searchTerms}">
        <c:param name="template" value="${jahia.includes.templatePath['/opensearch/resultsRss.jsp']}"/>
    </c:url>
    <Url type="application/rss+xml" template="${fn:escapeXml(url)}" title="${fn:escapeXml(title)}" />
