...
<div class="dex-TabPanelBottom">
  <div class="tabContent">
    ...
    <div id="content" class="fit w2">
          ...
          <th align="left" nowrap colspan="2">
              <span class="dex-PushButton">
                <span class="first-child">
                  <a title="<internal:engineResourceBundle resourceName='org.jahia.apply.button'/>" onclick="sendForm('showRevisionsList',''); return false;" href="#filter" class="ico-refresh"><internal:engineResourceBundle resourceName="org.jahia.apply.button"/></a>
                </span>
              </span>
          </th>
        </tr>
      ...
      <div class="head">
        <div class="object-title">
          <internal:engineResourceBundle resourceName="org.jahia.engines.version.nbOfRevisions"/>&nbsp;:&nbsp;<%=nbRevisions%>&nbsp;/&nbsp;<%=allRevisions.size()%>
        </div>
      </div>
      <%-- 
      <div id="operationMenu">
        <span class="dex-PushButton">
          <span class="first-child">
            <a href="javascript:handleCompare();" class="ico-compare" title="<internal:engineResourceBundle resourceName="org.jahia.engines.version.compare"/>">
              <internal:engineResourceBundle resourceName="org.jahia.engines.version.compare" defaultValue="Compare" /></a>
          </span>
        </span>
      </div>
      --%>
      <display:table name="revisions" class="evenOddTable" pagesize="20" requestURI="<%=requestURI%>" id="listItem" htmlId="listItem" style="width: 100%;" cellpadding="0" cellspacing="0" export="false">
        ...
        <display:column title='<%=JahiaResourceBundle.getEngineResource( "org.jahia.engines.version.operations", jParams, jParams.getLocale(),"Operations")%>' sortable="false" headerClass="lastCol" class="lastCol">
          <%if ( revEntrySet.getWorkflowState() > EntryLoadRequest.ACTIVE_WORKFLOW_STATE ){%>
            <a href="javascript:sendForm('showConfirmContainerUndoStaging');"><internal:engineResourceBundle resourceName="org.jahia.engines.version.undoStaging" defaultValue="undo staging"/></a>
          <%} else if ( allowRestoreOperation && count > 1){%>
            <a href="javascript:restoreVersion('<%=revEntrySet.toString()%>');"><internal:engineResourceBundle resourceName="org.jahia.engines.version.restore" defaultValue="restore" /></a>
          <%}%>
        </display:column>
      </display:table>
